...
function criaNuvens(){
  if(frameCount%60===0){
    nuvem=createSprite(larguraJogo,100,40,10); // MUDEI
...
function criaObstaculos(){
  if(frameCount%60===0){
    obstaculo=createSprite(larguraJogo, 162, 10, 50); // MUDEI
...
function setup() {
  ...
  chao=createSprite( larguraJogo/2,180,larguraJogo,20) // MUDEI
  chao.addAnimation("ground",imagemchao)
  chao.x=chao.width / 2
  
  chaoInvisivel=createSprite(larguraJogo/2,195,larguraJogo,20) // MUDEI
  ...
  reiniciar = createSprite(larguraJogo / 2, 140) // MUDEI
  reiniciar.addImage(reiniciarImg)
  reiniciar.scale=0.6
  
  fimDeJogo = createSprite(larguraJogo / 2, 100, 300, 400) // MUDEI
...
function draw() {
  ...
  if (estadoDoJogo === "jogar") {
    ...
    if(pontuacao>0&&pontuacao%1000===0){  // MUDEI
      ponto.play()
    }
    ...
    if (mousePressedOver(reiniciar) || touches.length > 0) { // MUDEI
      reset();
      touches = []
    }
  }
